...
export default function MemoryDetails4({ navigation }) {
  ...
  useEffect(() => {
    AsyncStorage.getItem("memoryEmotions")
      .then((storedText) => {
        if (storedText) {
  ...
  const handleTextChange = (text) => {
    setInputText(text);
    AsyncStorage.setItem("memoryEmotions", text)
      .then(() => {
    ...
    <FixWhiteSpace>
      <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
        ...
        >
          ...
          />
          <Image
            source={require("../assets/Images/memoryemotionstext.png")}
            style={{
              position: "absolute",
              top: windowHeight * 0.255,
